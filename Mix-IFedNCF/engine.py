...
class Engine(object):
    ...
    def __init__(self, config):
        ...
        # 【核心修改 1】：设置 reduction='none'，阻止 PyTorch 自动求平均，以便我们逐个样本乘上时长权重
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers):
        ...
        # 【核心修改 3】：计算加权 Loss
        # 1. 算基础 Loss (由于 reduction='none'，这里返回的是一个包含 batch_size 个 loss 的向量)
        base_loss = self.client_crit(ratings_pred.view(-1), ratings)
        
        # 2. 乘上停留时长权重 w (停留时间越长，惩罚/拉力越大)
        weighted_loss = base_loss * weights
        
        # 3. 将加权后的 loss 求平均，恢复标量状态
    ...
    def fed_train_a_round(self, user_ids, all_train_data, round_id, item_content):
        ...
        if self.config['clients_sample_ratio'] <= 1:
            # 【修复点】：改用当前传入池子的人数 len(user_ids) 来计算采样数
