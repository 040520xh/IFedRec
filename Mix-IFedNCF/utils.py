...
# ======= 追加到 Mix-IFedNCF/utils.py 最末尾 =======

def kuairec_weighted_sampling(df, num_negative, total_items):
    """
    专门为保留停留时长 weight 设计的负采样函数。
    返回格式：{user_id: [users, items, ratings, weights]} 完美适配修改后的 Dataset
    """
    all_train_data = {}
    grouped = df.groupby('user_id')
    for user_id, group in grouped:
        pos_items = group['video_id'].tolist()
        pos_weights = group['weight'].tolist()

        users = [user_id] * len(pos_items)
        items = pos_items[:]
        ratings = [1.0] * len(pos_items)
        weights = pos_weights[:] # 正样本拥有真实的连续权重！
        ...
        for _ in range(len(pos_items) * num_negative):
            ...
            ratings.append(0.0)
            weights.append(1.0) # 负样本基础权重设为1.0

        all_train_data[user_id] = [users, items, ratings, weights]
    return all_train_data

def build_test_set(df):
    """
    从数据集中剥离出测试集 (每个用户的最后两个交互视频)
    返回: (训练集 DataFrame, 测试集 DataFrame/Dict)
    """
    test_dict = {'uid': [], 'iid': []}
    train_df_list = []
    for uid, group in df.groupby('user_id'):
        if len(group) > 3:
            test_items = group.tail(2) # 取最后 2 个视频作为验证
